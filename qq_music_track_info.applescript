-- QQ Music Track Info Script (使用绝对路径修复)
-- 使用绝对路径来避免 BTT 权限问题

on getQQMusicTrackInfo()
    try
        ...
        if isQQMusic as integer > 0 then
            ...
            -- 获取艺术家
            set trackArtist to (do shell script "echo '" & mediaInfo & "' | /usr/bin/sed -n 's/.*\"artist\":\"\\([^\"]*\\)\".*/\\1/p'")
            
            -- 获取专辑
            set trackAlbum to (do shell script "echo '" & mediaInfo & "' | /usr/bin/sed -n 's/.*\"album\":\"\\([^\"]*\\)\".*/\\1/p'")
            
            if trackTitle is not "" and trackArtist is not "" then
                return trackArtist & " - " & trackTitle
            else if trackTitle is not "" then
                return trackTitle
            end if
        end if
...
end getQQMusicTrackInfo

-- 主逻辑
return getQQMusicTrackInfo() 